        ...
        .then((response) => {
          let sha = response.sha,
            ...
            decodedContent = decodeURIComponent(
              escape(window.atob(encodedContent))
            );
          ...
          encodedContent = window.btoa(
            unescape(encodeURIComponent(decodedContent))
          );
          ...
          success.json().then((res) => {
            ...
            if (success.status && success.status === 200) {
              message =
                'Link pushed: <a href="' +
                res.commit.html_url +
                '" target="_blank">Click</a> (Popup will auto close after 5 secs)';
